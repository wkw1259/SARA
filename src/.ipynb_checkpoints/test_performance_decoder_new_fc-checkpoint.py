...
# class Model_for_reward(nn.Module):
#     def __init__(self, base_model, config):
#         '''
#         计算alpha,beta,gamma权重
#         '''
#         super(Model_for_reward, self).__init__()
#         score_hidden_size = config.alpha_et_hidden_size   # 200
#         self.base_model = base_model
#         hidden_size = base_model.config.hidden_size
#         self.my_all_f = nn.Linear(3 * hidden_size, score_hidden_size)
        
#         self.relu = nn.ReLU()

#         self.output_f = nn.Linear(score_hidden_size, 3)
        
#         # self.my_alpha_f = nn.Linear(score_hidden_size, 1)
#         # self.my_beta_f = nn.Linear(score_hidden_size, 1)
#         # self.my_gamma_f = nn.Linear(score_hidden_size, 1)

#     def forward(self, input_ids, presumm_input, null_inputs, **generate_kwargs):
#         import torch.nn.functional as F

#         outputs = self.base_model(input_ids, **generate_kwargs)
#         presumm_outputs = self.base_model(presumm_input, **generate_kwargs)
#         null_outputs = self.base_model(null_inputs, **generate_kwargs)

#         # 先拼接三个类别的hidden layer
#         all_outputs = torch.cat([outputs, presumm_outputs, null_outputs], axis=-1)
#         all_outputs = self.my_all_f(all_outputs)
#         all_outputs = self.relu(all_outputs)
#         all_outputs = self.output_f(all_outputs)

#         weight, presumm_weight, null_weight = all_outputs[:,:,0], all_outputs[:,:,1], all_outputs[:,:,2]

#         concatenated_matrix = torch.cat((weight, presumm_weight), dim=1)
#         softmax_output = F.softmax(concatenated_matrix, dim=1)
#         alpha, beta = softmax_output[:, 0].unsqueeze(1), softmax_output[:, 1].unsqueeze(1)
#         gamma = torch.sigmoid(null_weight)

#         return alpha, beta, gamma, outputs, presumm_outputs, null_outputs
    
#     def generate(self, alpha, beta, gamma, outputs, presumm_outputs, null_outputs, **generate_kwargs):
...
if __name__ == "__main__":
    ...
    # model = Model_for_reward(base_model, args)  # 计算alpha,beta,gamma权重的model
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=500,   # int(num_training_steps//10),  # warmup步数取得总步数的10%
        num_training_steps=2000 # num_training_steps
    )
