...
class GenerationMixin:
    ...
    def generate(
        self,
        inputs: Optional[torch.Tensor] = None,
        null_inputs: Optional[torch.Tensor] = None,  ###cad
        presumm_input: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        null_inputs_model_kwargs = {"input_ids": null_inputs} #cad
        presumm_inputs_model_kwargs={"input_ids": presumm_input, "attention_mask":model_kwargs['presumm_attention_mask']}
        model_kwargs.pop('presumm_attention_mask')
        ...
        )  # TODO: something is problematic here, when alpha=0., this cannot produce valid null input, need to fix later  #cad
        presumm_tensor,model_input_name__, presumm_inputs_model_kwargs = self._prepare_model_inputs(
            None,generation_config.bos_token_id, presumm_inputs_model_kwargs
        )
        ...
        if null_inputs_model_kwargs.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask: #cad
        ...
        #if presumm_inputs_model_kwargs.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask: 
        #    presumm_inputs_model_kwargs["attention_mask"] = self._prepare_attention_mask_for_generation(
        #        presumm_tensor, generation_config.pad_token_id, generation_config.eos_token_id
        #    )
      
        '''
        if not torch.equal(model_kwargs["attention_mask"], presumm_inputs_model_kwargs["attention_mask"]):
            print(inputs_tensor,'inputs_tensor')
            print(presumm_tensor,'presumm_tensor')
            assert torch.equal(inputs_tensor, presumm_tensor)
            print("presumm_inputs_model_kwargs and model_kwargs attention_mask mismatch")
            print(model_kwargs.keys(),'model_kwargs.keys()')
            print(presumm_inputs_model_kwargs.keys(),'presumm_inputs_model_kwargs.keys()')
            print(model_kwargs["attention_mask"],'model_kwargs["attention_mask"]')
            print(presumm_inputs_model_kwargs["attention_mask"],'presumm_inputs_model_kwargs["attention_mask"]')
            print(model_kwargs["attention_mask"][0],'model_kwargs["attention_mask"][0]')
            print(model_kwargs["attention_mask"][1],'model_kwargs["attention_mask"][1]')
            print(presumm_inputs_model_kwargs["attention_mask"][0],'presumm_inputs_model_kwargs["attention_mask"][0]')
            exit()
        '''
        ...
        if self.config.is_encoder_decoder:
            ...
            null_input_ids = null_inputs_tensor #cad
            presumm_input_id = presumm_tensor
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            presumm_input_id, presumm_inputs_model_kwargs = self._expand_inputs_for_generation(
                input_ids=presumm_input_id,
                expand_size=generation_config.num_return_sequences,
                is_encoder_decoder=self.config.is_encoder_decoder,
                **presumm_inputs_model_kwargs,
            )

            # 13. run sample
            return self.sample(
                input_ids,
                presum_input_ids=presumm_input_id,
                null_input_ids=null_input_ids, #cad
                null_inputs_model_kwargs=null_inputs_model_kwargs,
                presumm_inputs_model_kwargs=presumm_inputs_model_kwargs,
                context_aware_decoding_alpha=generation_config.context_aware_decoding_alpha,
                cad_full=generation_config.cad_full,
                cad_salience=generation_config.cad_salience,
                logits_processor=logits_processor,
    ...
    def sample(
        self,
        input_ids: torch.LongTensor,
        presum_input_ids:Optional[torch.LongTensor] = None,
        null_input_ids: Optional[torch.LongTensor] = None,  #cad
        null_inputs_model_kwargs = None,
        presumm_inputs_model_kwargs =None,
        context_aware_decoding_alpha: Optional[float] = None,
        cad_full: Optional[float] = None,
        cad_salience: Optional[float] = None,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        logits_warper: Optional[LogitsProcessorList] = None,
        max_length: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            null_model_inputs = self.prepare_inputs_for_generation(null_input_ids, **null_inputs_model_kwargs) if context_aware_decoding_alpha > 0. else None #cad
            null_model_inputs = {f'null_{key}': null_model_inputs.pop(key) for key in list(null_model_inputs.keys())}
            presumm_model_inputs = self.prepare_inputs_for_generation(presum_input_ids,**presumm_inputs_model_kwargs)
            presumm_model_inputs = {f'presumm_{key}': presumm_model_inputs.pop(key) for key in list(presumm_model_inputs.keys())}
            
            combined_inputs = {**model_inputs, **null_model_inputs, **presumm_model_inputs}

            #if not torch.equal(model_inputs['attention_mask'], presumm_model_inputs['attention_mask']):
            #    print("Attention mask mismatch")
            #    exit()
            
            #print('model_inputs',model_inputs)
            #print('presumm_model_inputs',presumm_model_inputs)
            #print('\n\n\n')

            # forward pass to get next token
            outputs, presumm_outputs, null_outputs, weight = self(
                **combined_inputs,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
            ...
            presumm_outputs_logits = presumm_outputs.logits[:, -1, :] #cad

            # presumm_outputs = self(
            #         **presumm_model_inputs,
            #         return_dict=True,
            #         output_attentions=output_attentions,
            #         output_hidden_states=output_hidden_states,
            #       )
            #print('outputs',outputs)
            #print('presumm_outputs',presumm_outputs)
            #print('\n\n\n')
            
            #print('next_token_logits',next_token_logits)
            #print('presumm_outputs_logits',presumm_outputs_logits)
            
           
            #print(outputs.past_key_values[0][0],'outputs.past_key_values[0][0]')
            #if not torch.allclose(outputs.past_key_values[0][0], presumm_outputs.past_key_values[0][0], atol=1e-6):
            #    print(f"Difference found in past_key_values")
            #    exit()
            if context_aware_decoding_alpha > 0.:   #cad
                """
                ...
                # 当generate（）参数里面的num_return_sequences不等于1时，logits会生成很多个结果
                # def expand_to_match(tensor, reference):
                #     if tensor.size(0) < reference.size(0):
                #         # 计算需要重复的次数 (10 / 2 = 5)
                #         repeat_times = reference.size(0) // tensor.size(0)
                #         # 先插入序列维度 [2,50000] -> [2,1,50000]
                #         expanded = tensor.unsqueeze(1)  
                #         # 沿序列维度复制 [2,1,50000] -> [2,5,50000]
                #         expanded = expanded.repeat(1, repeat_times, 1)  
                #         # 合并batch和序列维度 [2,5,50000] -> [10,50000]
                #         return expanded.reshape(-1, tensor.size(-1))  
                #     else:
                #         return tensor
                
                # presumm_outputs_logits = expand_to_match(presumm_outputs_logits, next_token_logits)
                # null_next_token_logits = expand_to_match(null_next_token_logits, next_token_logits)

                ## 生成权重
                alpha_beta = weight[:, :2]  # 前两列 (alpha, beta)
                gamma = weight[:, 2]        # 最后一列 (gamma)
                # 对alpha和beta进行softmax（按行归一化）
                alpha_beta = torch.softmax(alpha_beta, dim=1)
                alpha = alpha_beta[:, 0].unsqueeze(1)  # alpha列，保持(batch_size, 1)形状
                beta = alpha_beta[:, 1].unsqueeze(1)   # beta列
                # 对gamma进行sigmoid
                gamma = torch.sigmoid(gamma).unsqueeze(1)

                next_token_logits = (1+gamma)*(alpha*next_token_logits+beta*presumm_outputs_logits)-gamma*null_next_token_logits

                #next_token_logits = next_token_logits * (context_aware_decoding_alpha+1.) - null_next_token_logits * context_aware_decoding_alpha
                # next_token_logits = (1+context_aware_decoding_alpha)*(cad_full*next_token_logits+cad_salience*presumm_outputs_logits)-context_aware_decoding_alpha*null_next_token_logits
                #print('null_next_token_logits',null_next_token_logits)
            else:
                next_token_logits = cad_full*next_token_logits+cad_salience*presumm_outputs_logits
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            presum_input_ids = torch.cat([presum_input_ids, next_tokens[:, None]], dim=-1) #cad
            ...
            presumm_inputs_model_kwargs = self._update_model_kwargs_for_generation(
                    presumm_outputs, presumm_inputs_model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
                ) #cad
            #print('model_kwargs',model_kwargs)
            #print('presumm_inputs_model_kwargs',presumm_inputs_model_kwargs)
            #print('\n\n\n')
            if context_aware_decoding_alpha > 0.:  #cad
